...
export type AugmentOps<
    Base extends OperationIndex,
    Augmentation extends OperationIndexAugmentation,
> = {
...
export type AugmentOp<Base extends Operation, Augmentation extends OperationAugmentation> = {
    [K in keyof Base | keyof Augmentation]: K extends keyof Base & keyof Augmentation & 'parameters'
...
export type AugmentParams<
    Base extends Operation['parameters'],
    Augmentation extends ParamAugmentation,
> = {
    [K in keyof Base | keyof Augmentation]: K extends keyof Augmentation & 'query'
        ? AugmentRecord<
              Exclude<Base['query'], undefined>,
              Exclude<Augmentation['query'], undefined>
          >
...
type AugmentRecord<Base, Augmentation> = 1 extends 0
    ? never
    : {
          [K in keyof Base | keyof Augmentation]: K extends keyof Augmentation
              ? Augmentation[K]
              : K extends keyof Base
                ? Base[K]
                : never;
